...
const RENTAL_SOURCES = [
  ...
  {value:"szpital",label:"🏥 Szpital",color:"#3E6FB0"},
...
const Btn = ({children,onClick,variant="primary",small,disabled,style={}}) => {
  ...
  const v={primary:{background:"#3E6FB0",color:"#fff"},secondary:{background:dk?"#1E2F4A":"#E1E9F5",color:"#3E6FB0"},orange:{background:"#F4A261",color:"#fff"},danger:{background:dk?"#3A1C1C":"#FEE2E2",color:"#E05C5C"}};
...
const Card = ({children,onClick,style={}}) => {
  const dk=useContext(DarkCtx);
  return <div onClick={onClick} style={{background:dk?"#18202F":"#fff",borderRadius:16,padding:"18px 20px",marginBottom:12,boxShadow:dk?"0 2px 14px rgba(0,0,0,.22)":"0 2px 14px rgba(16,40,40,.06)",border:dk?"1px solid #24304A":"1px solid rgba(16,40,40,.04)",cursor:onClick?"pointer":"default",...style}}>{children}</div>;
};
const Badge = ({children,color="#3E6FB0"}) => <span style={{background:color+"28",color,borderRadius:8,padding:"4px 10px",fontSize:12,fontWeight:700}}>{children}</span>;
const Empty = ({text}) => <div style={{textAlign:"center",padding:"40px 20px",color:"#7A8FA6",fontSize:14}}>{text}</div>;
const Av = ({name}) => { const i=(name||"?").split(" ").slice(0,2).map(w=>w[0]).join("").toUpperCase(); return <div style={{width:44,height:44,borderRadius:12,background:"#E1E9F5",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:15,color:"#3E6FB0",flexShrink:0}}>{i}</div>; };

// Etykieta nagłówka sekcji — zastępuje powtarzany wszędzie inline styl
const SectionLabel = ({children,style={}}) => {
  const dk=useContext(DarkCtx);
  return <div style={{fontSize:11,fontWeight:700,color:dk?"#6B84AC":"#7A8FA6",textTransform:"uppercase",letterSpacing:.5,marginBottom:10,...style}}>{children}</div>;
};

// Karta metryki (etykieta + duża wartość), do rzędów statystyk
const StatCard = ({label,value,accent,style={}}) => {
  const dk=useContext(DarkCtx);
  return <div style={{background:dk?"#18202F":"#fff",borderRadius:16,padding:"16px 18px",boxShadow:dk?"0 2px 14px rgba(0,0,0,.22)":"0 2px 14px rgba(16,40,40,.06)",border:dk?"1px solid #24304A":"1px solid rgba(16,40,40,.04)",...style}}>
...
const ListRow = ({icon,iconBg,iconColor,title,subtitle,trailing,onClick,last,style={}}) => {
  ...
  return <div onClick={onClick} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 2px",borderBottom:last?"none":`1px solid ${dk?"#20302E":"#F0F3F5"}`,cursor:onClick?"pointer":"default",...style}}>
    {icon&&<div style={{width:34,height:34,borderRadius:10,background:iconBg||(dk?"#1E2F4A":"#E1E9F5"),display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:16,color:iconColor||"#3E6FB0"}}>{icon}</div>}
